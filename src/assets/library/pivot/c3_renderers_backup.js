    ...
    callWithJQuery(function($, c3) {
        ...
        makeC3Chart = function(chartOpts) {
            ...
            return function(pivotData, opts) {
                ...
                } else if (chartOpts.type === "timeseries") {
                    console.log('time series chart 1 *********');
                    timeseriesData = {
                        x: {},
                        y: {},
                        t: {}
                    ...
                    for (i = 0, len = rowKeys.length; i < len; i++) {
                        ...
                        for (j = 0, len1 = colKeys.length; j < len1; j++) {
                            ...
                            if (agg.value() != null) {
                                ...
                                if ((base3 = timeseriesData.x)[series] == null) {
                                    base3[series] = [];
                                }
                                if ((base4 = timeseriesData.y)[series] == null) {
                                    base4[series] = [];
                                }
                                y = (ref2 = vals[0]) != null ? ref2 : 0;
                                x = (ref3 = vals[1]) != null ? ref3 : 0;
                                timeseriesData.y[series].push(y);
                                timeseriesData.x[series].push(x);
                                if ((base5 = timeseriesData.t)[series] == null) {
                                    base5[series] = {};
                                }
                                if ((base6 = timeseriesData.t[series])[x] == null) {
                                    base6[x] = {};
                                }
                                timeseriesData.t[series][x][y] = agg.value();
                            }
                        }
                    }
                ...
                if (chartOpts.type === "scatter") {
                    ...
                    console.log('time series chart 2 *********');
                    // xs = {};
                    numSeries = 0;
                    dataColumns = [];
                    for (s in timeseriesData.x) {
                        numSeries += 1;
                        // xs[s] = s + "_x";
                        // dataColumns.push([s + "_x"].concat(timeseriesData.x[s]));
                        dataColumns.push(["x"].concat(timeseriesData.x[s]));
                        dataColumns.push([s].concat(timeseriesData.y[s]));
                    }
                    // params.data.xs = xs;
                    params.data.x = 'x';
                    params.data.columns = dataColumns;
                    ...
                    params.axis.x.type = chartOpts.type;
                    if (numSeries === 1) {
                        params.legend = {
                            show: false
                        };
                    ...
                    /* params.tooltip.format = {
                        ...
                        value: function(a, b, c, d, e) {
                            var ref4;
                            ref4 = e[0], series = ref4.name, y = ref4.value, x = ref4.x;
                            return formatter(timeseriesData.t[series][x][y]);
                        }
                    }; */
                } else {
        ...
        return $.pivotUtilities.c3_renderers = {
            ...
            "Area Step Chart": makeC3Chart({
                type: "area-step"
            }),
            "Area Spline Chart": makeC3Chart({
                type: "area-spline"
            }),
            ...
            // added by User
            // TODO - if you are updating this library then you will have to add below codes to the latest one
            "Pie Chart": makeC3Chart({
                type: "pie"
            }),
            "Donut Chart": makeC3Chart({
                type: "donut"
            }),
            /* "Gauge Chart": makeC3Chart({
                type: "gauge"
            }), */
            "Spline Chart": makeC3Chart({
                type: "spline"
            }),
            "Step Chart": makeC3Chart({
                type: "step"
            }),
            "Time Series": makeC3Chart({
                type: "timeseries"
            })
        };
    });
