...
import { statementPreviewData } from './preview.interface';
...
@Component({
  ...
  providers: [
    PreviewService, CommonHttpAdapterService, CommonService,
    RxJSHelper
  ]
})
export class PreviewComponent implements OnInit, OnDestroy {
  ...
  previewPageLoad = false;
  previewRepoError = false;
  ...
  /** To get category and sub category */
  ...
  getStatementPreviewDetails() {
    ...
    this.previewService.getStatementData().subscribe(
      res => {
        this.previewPageLoad = true;
        this.statementPreviewData = res;
        // response {"investments":[],"banks":[],"cards":[],"loans":[],"fixedDeposits":[]}
  ...
  investmentRow(index, investment) {
  ...
  showRemainingHoldingAssetFields(investmentIndex: number, assetIndex: number, asset) {
    document.getElementById('assetError_' + investmentIndex + '_' + assetIndex).innerHTML = '';
  ...
  saveScx(parentIndex: number, index: number, scx) {
    ...
    updatedScxDetails[CommonConstant.FLOW] = 'pimoney';
    ...
    this.previewService.updateInvestmentsData(updatedScxDetails).subscribe(
      res => {
        if (res[CommonConstant.ERROR_CODE] === 0) {
          // hiding loader and editable fields
  ...
  /**To generate request for confirm and rejeact investement data*/
  ...
  confirmInvestmentAccounts() {
    document.getElementById('investmentError').innerHTML = '';
    ...
    if (confirmInvestmentAccountsData) {
      ...
      this.previewService.confirmAccountStatementData(confirmInvestmentAccountsData).subscribe(
        res => {
          if (res[CommonConstant.ERROR_CODE] === 0) {
            ...
            this.investmentDataLoading = false;
            document.getElementById('investmentError').innerHTML = res[CommonConstant.MESSAGE];
          }
        },
        error => {
          this.investmentDataLoading = false;
          document.getElementById('investmentError').innerHTML = 'Something went wrong..Please try again';
        }
      )
    }
  ...
  rejectInvestmentAccounts() {
    ...
    if (rejectInvestmentAccountsData) {
      this.previewService.rejectAccountStatementData(rejectInvestmentAccountsData).subscribe(
        res => {
          if (res[CommonConstant.ERROR_CODE] === 0) {
        ...
        error => {
          document.getElementById('investmentError').innerHTML = 'Something went wrong';
        }
      )
    }
  }
  ...
  /** To confirm credit card account */
  confirmCreditCardAccounts() {
    ...
    if (confirmCreditcardAccountsData) {
      this.previewService.confirmAccountStatementData(confirmCreditcardAccountsData).subscribe(
        res => {
  ...
  rejectCreditCardAccounts() {
    ...
    const confirmCreditcardAccountsData = this.makeRequestForCreditcardConfirmReject();
    if (confirmCreditcardAccountsData) {
      this.previewService.rejectAccountStatementData(confirmCreditcardAccountsData).subscribe(
        res => {
          this.creditcardDataLoading = false;
          if (res[CommonConstant.ERROR_CODE] === 0) {
            // refresh preview page
  ...
  editCreditcardTransaction(i, transaction, event) {
    if (!transaction.editTransaction) {
      ...
      this.editTransactionObj['updatedTransDate'] = this.editTransactionObj.transDate;
    ...
    this.commonService.updateAccounts(updatedTxnDetails).subscribe(
      res => {
        if (res[CommonConstant.ERROR_CODE] === 0) {
          ...
          this.statementPreviewData.cards[index].transactions[childIndex] = res.data;
        } else {
          // hiding loader and editable fields
          transaction.transactionDataLoading = false;
          document.getElementById('txnError_' + index + '_' + childIndex).innerHTML = '';
  ...
  confirmLoanAccounts() {
    ...
    if (confirmLoanAccountsData) {
      this.previewService.confirmAccountStatementData(confirmLoanAccountsData).subscribe(
        res => {
  ...
  rejectLoanAccounts() {
    ...
    if (confirmLoanAccountsData) {
      this.previewService.rejectAccountStatementData(confirmLoanAccountsData).subscribe(
        res => {
          this.loanDataLoading = false;
          if (res[CommonConstant.ERROR_CODE] === 0) {
            // refresh preview page
